...
import { switchLang, useReactiveLang, LANG_FLAGS, LANG_LABELS, type Lang } from "@/lib/utils";
...
export default function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-white/90 backdrop-blur-xl shadow-sm border-b border-gray-100/80">
        ...
        <div className="hidden md:flex items-center gap-7 lg:gap-9">
          {navItems.map((item) => (
            <a
              key={item.key}
              href={item.href}
              className="relative text-sm tracking-wider font-medium text-gray-700 hover:text-teal-700 transition-colors duration-300 group"
            >
              {t(item.key)}
          ...
          <div ref={langRef} className="relative">
            <button
              ...
              className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full border border-gray-300 text-gray-600 hover:border-teal-600 hover:text-teal-700 transition-all"
            >
          ...
          {content?.agent?.social?.whatsapp && (
            <a
              ...
              className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-full border border-gray-300 text-gray-600 hover:border-green-500 hover:text-green-600 transition-all"
              aria-label="WhatsApp"
            >
          ...
          <a
            href="#contact"
            className="px-5 py-2.5 text-sm font-medium rounded-lg bg-teal-700 text-white hover:bg-teal-600 shadow-sm transition-all duration-300"
          >
        ...
        <div className="flex md:hidden items-center gap-2">
          <div ref={mobileLangRef} className="relative">
            <button
              ...
              className="flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full border border-gray-300 text-gray-600"
            >
              <span>{LANG_FLAGS[lang]}</span>
              <svg className={`w-3 h-3 transition-transform ${mobileLangDropdown ? "rotate-180" : ""}`} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M6 9l6 6 6-6" />
              </svg>
            </button>
