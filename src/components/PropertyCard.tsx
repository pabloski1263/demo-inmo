...
export default function PropertyCard({ property, index = 0 }: PropertyCardProps) {
  ...
  const statusKey = statusKeyMap[property.status] || "property.pending";
    ...
    >
      ...
      <div className="relative h-48 sm:h-56 overflow-hidden bg-gray-100">
        ...
        <div className={`absolute top-3 left-3 ${statusColors[property.status] || "bg-gray-500"} text-white text-[10px] font-medium px-2.5 py-1 rounded-full uppercase tracking-wider`}>
          {lang === "en"
            ? ({ "for-sale": "For Sale", "for-rent": "For Rent", sold: "Sold", pending: "Pending" } as Record<string, string>)[property.status]
            : ({ "for-sale": "En Venta", "for-rent": "En Arriendo", sold: "Vendido", pending: "Pendiente" } as Record<string, string>)[property.status]}
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5">
        <p className="text-xl font-bold text-gray-900 mb-1">
          ...
          {property.status === "for-rent" && <span className="text-sm font-normal text-gray-500">/mes</span>}
        </p>
